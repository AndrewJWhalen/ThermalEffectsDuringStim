...
switch point
    case 1
        [vset,mnhset]=meshgrid(linspace(-65,-58,xnstep),linspace(0,0.07,ynstep));
        [v0list, neq, meq, heq, ~, ~] = equilibrium(profile_0);
    case 3
        [vset,mnhset]=meshgrid(linspace(-42,-40,xnstep),linspace(0.5,0.52,ynstep));
...
for i = 1:size(vset,1)
    for ii = 1:size(vset,2)
        u1(i,ii) = double(subs(subs(dotv,m,mnhset(i,ii)),v,vset(i,ii)));
        u2(i,ii) = double(subs(subs(dotm,m,mnhset(i,ii)),v,vset(i,ii)));
...
for val_x = v_nc
    ...
    mnh_nc = [mnh_nc,vpasolve(subs(dotv,v,val_x)==0,m)];
...
hold on
plot(min(vset,[], 'all'):0.01:max(vset,[], 'all'),subs(alpha_m/(alpha_m+beta_m),v,min(vset,[], 'all'):0.01:max(vset,[], 'all')))
