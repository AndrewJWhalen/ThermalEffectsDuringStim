...
switch point
    case 1
        [vset,mnhset]=meshgrid(linspace(-65,-58,xnstep),linspace(0.95,1,ynstep));
        [v0list, neq, meq, heq, ~, ~] = equilibrium(profile_0);
    case 3
        [vset,mnhset]=meshgrid(linspace(-42,-40,xnstep),linspace(0.24,0.27,ynstep));
...
for i = 1:size(vset,1)
    for ii = 1:size(vset,2)
        u1(i,ii) = double(subs(subs(dotv,h,mnhset(i,ii)),v,vset(i,ii)));
        u2(i,ii) = double(subs(subs(doth,h,mnhset(i,ii)),v,vset(i,ii)));
...
for val_x = v_nc
    ...
    mnh_nc = [mnh_nc,vpasolve(subs(dotv,v,val_x)==0,h)];
...
hold on
plot(min(vset,[], 'all'):0.01:max(vset,[], 'all'),subs(alpha_h/(alpha_h+beta_h),v,min(vset,[], 'all'):0.01:max(vset,[], 'all')))
