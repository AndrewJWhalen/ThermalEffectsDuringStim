function [r,cur]=model_int_var_o2_current_temp_cotrans_bath(x,bath,pump, nmod, mmod, hmod, gkmod, gnmod, leak_k, leak_na, leak_cl, o2D,temp,cotrans1,cotrans2)
%% Parameters
   % Basic HH paramters
...
%% Cloride transporter (mM/s)
   fKo = (1/(1+exp((16-Ko)/1)));
   FKCC2 = Ukcc2*log((Ki*Cli)/(Ko*Clo));
   FNKCC1 = Unkcc1*fKo*(log((Ki*Cli)/(Ko*Clo))+log((Nai*Cli)/(Nao*Clo)));
